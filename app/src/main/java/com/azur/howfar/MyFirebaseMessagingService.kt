...
import com.azur.howfar.models.ChatAndMessage
...
class MyFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        if (remoteMessage.data.isNotEmpty()) {
            val json = remoteMessage.data["body"]
            val dataJson = remoteMessage.data["data"]
            val messageType = remoteMessage.data["view"]
            when (messageType) {
                "Message" -> {
                    val data = Gson().fromJson(dataJson, ChatAndMessage::class.java)
                    notifierHelper.listenForMessage(data.chatData)
                    Log.d(TAG, "Message data payload 1: ******************** ${remoteMessage.data}")
                }
                "call incoming" -> {
                    val activityIntent = Intent(this, IncomingCallDialog::class.java).apply {
                        putExtra("callData", dataJson)
                    }
                    val activityPendingIntent =
                        PendingIntent.getActivity(this, 0, activityIntent, PendingIntent.FLAG_UPDATE_CURRENT)
                    AppNotificationManager(this).createNormalNotification(
    ...
    companion object {
        ...
        fun subscribeToTopic(topic: String, baseContext: Context) {
